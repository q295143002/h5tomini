...
import RequestManager from "./requestManager.js"; // import util from '../util';
...
function request(obj, initUserData) {
  ...
  let queryComponents = []; // 加入请求里的param
  ...
  if (globalData.hd_origin) queryComponents.push("hd_origin=" + globalData.hd_origin); // todo config配置 暂时没有 先写个微信的beta,这个到时候在

  let url = (obj.host ? obj.host : Config.settings.requestDomain) + service + '?' + queryComponents.join('&');
  url = decodeURIComponent(url); // 处理某些业务线需要在header中传参数的情况
